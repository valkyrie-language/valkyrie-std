...
| R5 | **单一平台制品** | 逻辑 + UI wire + 元数据 **全部编入该平台交付物**；**禁止**独立 `ui.bin`、`host.bin`、`host/*.bin`、`libasgard_host` 等侧车 |
...
编译期 AWSL + V
  ...
  平台制品（一个文件承载全部尾段嵌入）
       │
       ├─ ASGARDNT 段：native AOT 逻辑（Android/iOS/桌面）
       └─ asgard ui 段（魔数 `ASGARDUI`）：RenderIR / UI wire
       │
       ▼
