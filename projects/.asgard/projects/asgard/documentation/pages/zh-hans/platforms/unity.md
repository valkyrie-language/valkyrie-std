...
- Unity 插件包 `valkyrie.unity` 为 **DLL-only UPM**（无 C# 源码）；由 V 项目 `valkyrie.v/projects/valkyrie.unity` 编译并按 `[export(unity.runtime|unity.editor)]` 分区落盘
...
**`valkyrie.unity` 为独立 Git 发布仓**（仅 `package.json` + `Runtime/`、`Editor/` 下 DLL，建议 Git LFS）。V 源码在 `valkyrie.v/projects/valkyrie.unity`。
