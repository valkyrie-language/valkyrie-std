...
micro decode_constant_entry(data: [i32], offset: usize) -> JvmBinaryResult<JvmDecoded<JvmBinaryConstant>> {
    ...
    if tag == 1 {
        let length_result = jvm_read_u16_be(data, offset + 1)?
        let utf8_length: usize = length_result.value as usize
...
micro decode_attribute(data: [i32], offset: usize) -> JvmBinaryResult<JvmDecoded<JvmBinaryAttribute>> {
    ...
    let length_result = jvm_read_u32_be(data, pos)?
    let attr_length: usize = length_result.value as usize
    pos = pos + 4
