...
| 名称         | 角色                                                                                      |
| ---------- | --------------------------------------------------------------------------------------- |
| **Asgard** | **GUI 应用框架 + 用户 CLI**（`asgard build` / `dev` / `pack` / `plan`；widget、AWSL、运行时） |
| **VOA**    | **Rust 编译实现**（crate `voa`，`asgard.config.v`；无独立 `voa` 可执行文件） |
...
1. **按 platform 一份制品（矩阵，非一二进制通吃）** — 同一业务源码可对多个 `platform` 各出一份交付物；**每一次** `platform` 构建仍是「逻辑 + UI wire 编入该平台制品尾段」。禁止把「一次开发」理解成「一个二进制跑全平台」。部署时用 [deploy profile](deploy-profiles.md) 选择拓扑（一个或多个制品）。禁止 `ui.bin`、`host.bin`、`libasgard_host`（见 [AOT 原则](aot-principles.md)）。
...
5. **平台原生优先，Bifrost 并行** — 宿主默认用**原生**视图 shim 消费编入制品的 UI wire；`asgard.ui` V 源码**不**上设备。同时允许 `asgard.bifrost` 作为并行自绘后端，承接跨平台统一自绘场景（见 [UI 渲染策略](ui-rendering.md)）。
6. **Asgard/VOA ∥ Atlas** — 客户端 GUI 走本管线；服务端 / server 岛走 Atlas（`atlas.config.von`）。二者由 deploy profile 组合，不合成 LiveView/SSR-first。
...
flowchart TB
    subgraph author [Asgard 应用]
        AWSL[".awsl"]
        VSrc[".v / script"]
        CFG["asgard.config.v platform + build.mode"]
    end
...
| `asgard.config.v`    | `platform` 选字节码后端；`build.mode` 控制 debug 侧车 |
...
| 项 | 目标架构 | 当前实现 |
|:---|:---|:---|
| 每 platform 一份制品（矩阵） | 无侧车 bin/so；多平台 = 多制品 | **已实现** — dex / Mach-O / wasm；**非**一二进制通吃 |
| Deploy profile 编排 | `asgard plan` + 构建编排 | 校验/矩阵打印 **已实现**；编排 **planned** |
| SSG 默认 | 构建期静态 HTML | **已实现** |
| Server 岛（Atlas 片段） | 请求时 HTML partial | **planned** |
