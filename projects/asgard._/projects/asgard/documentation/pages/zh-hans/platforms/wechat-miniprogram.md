...
`asgard.config.v`（`target = "wasm32-unknown-miniprogram-wasm"`；独立 MP prelude，shim 适配微信）：
...
`asgard build` 默认使用 `asgard.config.v` 的 `target`（`wasm32-unknown-miniprogram-wasm`）。
