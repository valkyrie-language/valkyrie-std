...
当前磁盘上的实现仍然主要集中在 [_.v](../../../../source/_.v)。
...
- workspace 与单项目分发
- 宿主桥调用（已移除，改由 `nyar_driver.v` 内自包含编译）
...
- 调用 `build_planner.v` / `nyar_driver.v` 完成后端执行
...
自举路径不再使用 `host_bridge.v` 或任何 CLR 外部 bridge（含 `LoL.Legion.HostBridge`、`clr_source_compile_project`、`clr_host_pack_project`）。

`nyar_driver.v` 的 `nyar_driver_compile_project` 必须在产物进程内完成 `FrontendBuildOutput → nyar-driver`。
...
- [_.v](../../../../source/_.v)
- [manifest.v](../../../../source/manifest.v)
- [build_context.v](../../../../source/build_context.v)
- [build_planner.v](../../../../source/build_planner.v)
