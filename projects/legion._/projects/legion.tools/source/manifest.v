...
# Explicit while + cardinal index — CLR anon-micro→Delegate lowering still emits
# uninitialized int32 locals into Option_map/Iterator_filter (InvalidProgram).
micro legion_collect_build_targets(value: VonValue) -> [LegionBuildTarget] {
    ...
    while i < items.length() {
        ...
        if target_name.length() > 0 {
            ...
            result = push(result, LegionBuildTarget {
                name: target_name,
                options: options
            })
        }
...
micro legion_collect_publish_targets(value: VonValue, default_version: utf8) -> [LegionPublishTarget] {
    ...
    while i < items.length() {
        ...
        if target_name.length() > 0 && publish_type.length() > 0 {
            result = push(result, LegionPublishTarget {
                target: target_name,
...
micro legion_collect_dependencies(deps_value: VonValue) -> [LegionDependency] {
    ...
    while i < fields.length() {
        ...
        if field.name != "core" && field.name != "std" && field.name.length() > 0 {
            ...
            result = push(result, LegionDependency {
                name: field.name,
                version: version,
                abi: abi
            })
