...
micro nyar_driver_compile_project(project_dir: utf8, target: utf8, output: utf8, verbose: bool) -> i32 {
    let plan_path: utf8 = output + "/compile-plan.txt"
    if !std.io.file_exists(plan_path) {
        std.io.error("错误：找不到 compile-plan.txt，无法执行 nyar-driver 编译")
        if verbose {
    ...
    if !result.ok {
        if verbose {
            std.io.print_line("nyar-driver 编译失败：" + result.error)
    ...
    if verbose {
        std.io.print_line("nyar-driver 编译完成")
