...
自举目标项目固定为 `valkyrie.v/projects/legion.tools`（Valkyrie 重写的 `legion` CLI / 编译器前端）。

L2 分 **两条并行轨**，共享同一源码与模块系统前置门，但产物与发布平台不同：

| 轨 | 目标 | 产物形态 | 主要发布平台 | 验收脚本 |
|:---|:---|:---|:---|:---|
| **CLR** | `clr` → `clr-microsoft-unknown-managed` | `legion.exe` + `.msil` | NuGet（.NET 工具链） | `bootstrap-clr.mjs` |
| **Node** | `node` → `wasm32-node-unknown-wasm` | `legion.mjs` + `legion.wasm` | **npm**、**JSR** | `bootstrap-node.mjs` |

任何声称「已完成 L2 自举」的结论，必须在对应轨上同时满足 **模块系统前置门**、**seed→v1→v2 链** 与 **产物比对规则**；否则视为假自举。

> CLR：`node scripts/bootstrap-clr.mjs`、`legion bootstrap --project projects/legion.tools`、CI `bootstrap.yml`  
> Node：`node scripts/bootstrap-node.mjs`（npm / JSR 分发轨，诚实失败验收）
...
2. **`v1 → v2` 必须由 v1 进程内的编译管线完成**（`legion build` / `nyar_driver_compile_project`），禁止 Rust/C# driver、HostBridge、环境变量委托。
...
- `valkyrie-wasi-host` 源文件驱动链（WASI wasm seed 再编译）
...
| 路径 | 要求 |
|:---|:---|
| `valkyrie.v/legions.von` | 超 workspace 清单存在 |
| `projects/legion.tools/legion.von` | `name: "legion.tools"` |
| `projects/nyar/legion.von` | `name: "nyar"` |
| `projects/std/legion.von` | `name: "std"` |
| `projects/legion.tools/source/build_context.v` | 显式 `using nyar;` 与目标解析辅助函数 |
...
**不得**再包含 `projects/micro_compiler`。
...
| 层级 | 文件 | 作用 |
|:---|:---|:---|
| 超 workspace | `valkyrie.v/legions.von` | 嵌套引用 `.asgard` / `.nyar` / `.legion` 等分发入口 |
| 子 workspace | 如 `projects/.legion/legions.von` | 单独开发 `legion.tools` 等工具包 |
...
| 源码 → v1.clr | `seed build projects/legion.tools --target clr` 产出 CLI | 非零退出 |
...
./target/release/legion bootstrap --project ../valkyrie.v/projects/legion.tools
...
# workspace / 规划验证
legion check projects/legion.tools --target node
legion check projects/legion.tools --target clr
```
