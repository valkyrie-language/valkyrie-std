...
本文定义 `projects/nyar.language/source/*` 的长期目录布局与共享约束（仓库路径：`valkyrie.v/projects/nyar._/projects/nyar.language/source/*`）。
...
`source/valkyrie` 是 `Valkyrie` 前端本体（语言名 Valkyrie，源码后缀 `.v`）。Valkyrie AST/CST 归属 `std.data.text.valkyrie`，本目录做前端消费与 HIR/MIR，不把 AST 长期私有在 language 内。
