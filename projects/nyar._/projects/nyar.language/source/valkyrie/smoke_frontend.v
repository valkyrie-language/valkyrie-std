...
micro smoke_capability_error(source: utf8, file_count: usize, dependency_count: i32) -> utf8? {
    if dependency_count > 0 {
        return "依赖闭包超出 smoke 子集：暂不支持 workspace 依赖包"
    }
    if file_count == 0 {
        return "源码闭包为空"
    }
    if file_count > 8 {
        return "源码文件数量超出 smoke 子集上限"
    }
    if contains_forbidden_marker(source, "<%") {
        return "暂不支持 arch 模板预处理（<% match arch %>）"
    }
    if contains_forbidden_marker(source, "structure ") {
        return "暂不支持 structure 声明"
    }
    if contains_forbidden_marker(source, "trait ") {
        return "暂不支持 trait"
    }
    if contains_forbidden_marker(source, "using legion") {
        return "暂不支持 legion.tools 级依赖解析"
    }
    if !source.contains("[main]") {
        return "smoke 子集要求存在 [main] 入口"
    }
    if !source.contains("console_write_line(") {
        return "smoke 子集要求 main 调用 console_write_line(...)"
    }
    return null
}
...
micro compile_source_to_build_output(combined_source: utf8, file_count: usize) -> SmokeCompileResult {
    let capability_error: utf8? = smoke_capability_error(combined_source, file_count, 0)
