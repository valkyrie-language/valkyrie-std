    ...
    match clr_read_u32_le(data, 60) {
        case Fine(pe_offset_u32):
            ...
            if !clr_check_pe_magic(data, pe_offset) {
                return Fail(clr_binary_diagnostic("PE 魔数不匹配，期望 PE\\0\\0", pe_offset))
            }
                    ...
                    match clr_read_u16_le(data, metadata_offset + 6) {
                        case Fine(minor_version):
                            match clr_read_u32_le(data, metadata_offset + 12) {
                                case Fine(version_len_u32):
                                    let version_len: usize = version_len_u32 as usize

                                    match clr_read_utf8(data, metadata_offset + 16, version_len) {
                                        case Fine(version_string):
                                            let version_end: usize = metadata_offset + 16 + version_len
                                                                    ...
                                                                    case Fine(stream_offset):
                                                                        match clr_read_u32_le(data, stream_pos + 4) {
                                                                            case Fine(stream_size):
                                                                                ...
                                                                                let name_byte_len: usize = clr_align_4(ni + 1)
                                                                                stream_pos = stream_pos + 8 + name_byte_len
                                                                            case Fail(error):
                                                                                return Fail(error)
                                                                        }
                                                                    case Fail(error):
