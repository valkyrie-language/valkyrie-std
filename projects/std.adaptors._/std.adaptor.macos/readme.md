...
| 文件       | 绑定类型            | 覆盖 API                                                                            |
|:-----------|:--------------------|:------------------------------------------------------------------------------------|
| `darwin.v` | `[c]` / `[syscall]` | Foundation / CoreFoundation / CoreGraphics / ObjC Runtime；Darwin/Mach `[syscall]` |

### Darwin API

标准库用 `[c]` 绑定 Apple Framework / ObjC Runtime，用 `[syscall]` 走内核入口；不依赖 libSystem：
...
用户代码若需 libSystem，可自行使用 `[c("libSystem", ...)]` FFI。
...
| 注解                 | 语义                                              |
|:---------------------|:--------------------------------------------------|
| `[c("lib", "func")]` | C 调用约定：Framework / ObjC（或用户自选的 libSystem） |
