# macOS network host_provider for std.network（仅 Darwin syscall）

namespace std.adaptor.macos.network;

[host_provider(std::network::net_socket_create)]
micro net_socket_create(domain: i32, type_: i32, protocol: i32): i32 {
    return sys_darwin_socket(domain, type_, protocol)
}
...
micro net_connect(fd: i32, host: utf8, port: i32): i32 {
    let _host: utf8 = host
    let _port: i32 = port
    return sys_darwin_connect(fd, 0, 16)
}
